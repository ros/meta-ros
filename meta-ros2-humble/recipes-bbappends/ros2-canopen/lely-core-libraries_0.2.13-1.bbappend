...
# CMake Error:
# Running
#  'ninja' '-C' 'TOPDIR/tmp-glibc/work/core2-64-oe-linux/lely-core-libraries/0.2.13-1-r0/build' '-t' 'recompact'
# failed with:
...
do_install:append() {
    ...
    sed -i -e '1s|^#!.*|#!${bindir}/env python3|' ${D}${ros_bindir}/cogen
    sed -i -e '1s|^#!.*|#!${bindir}/env python3|' ${D}${ros_libdir}/cogen/cogen
}
